(function () {
  ...
  async function verifyLicenseToken(token, machineCode) {
    try {
      ...
      if (payload.machineCode !== machineCode) return { ok: false, reason: "\x6d\x61\x63\x68\x69\x6e\x65\x20\x63\x6f\x64\x65\x20\x74\x69\x64\x61\x6b\x20\x63\x6f\x63\x6f\x6b" };
      if (Date.now() > payload.expiry) return { ok: false, reason: "\x6c\x69\x73\x65\x6e\x73\x69\x20\x73\x75\x64\x61\x68\x20\x65\x78\x70\x69\x72\x65\x64" };
  ...
  function formatExpiry(ts) {
    return new Date(ts).toLocaleDateString("\x69\x64\x2d\x49\x44", { day: "\x6e\x75\x6d\x65\x72\x69\x63", month: "\x6c\x6f\x6e\x67", year: "\x6e\x75\x6d\x65\x72\x69\x63" });
  }
        ...
        func: (machineCode) => {
          ...
          return new Promise((resolvePrompt) => {
            ...
            window.__tukangActivationShowSuccess = (expiry, daysRemaining) => {
              ...
              const expDate = new Date(expiry).toLocaleDateString("\x69\x64\x2d\x49\x44", { weekday: "\x6c\x6f\x6e\x67", day: "\x6e\x75\x6d\x65\x72\x69\x63", month: "\x6c\x6f\x6e\x67", year: "\x6e\x75\x6d\x65\x72\x69\x63" });
    ...
    if (msg.action === "\x63\x68\x65\x63\x6b\x4c\x69\x63\x65\x6e\x73\x65") {
      (async () => {
        try {
          const machineCode = await generateMachineCode();
          const { license_token } = await chrome.storage.local.get("license_token");
          if (!license_token) { sendResponse({ ok: false, reason: "\x6e\x6f\x5f\x6c\x69\x63\x65\x6e\x73\x65" }); return; }
      ...
      func: (expiryStr, daysRemaining) => {
        ...
        return new Promise((resolve) => {
          ...
          settingsBtn.onclick = () => {
            ...
            const modules = [
              { key: "\x64\x65\x6c\x61\x79\x5f\x64\x6f\x6b\x75\x6d\x65\x6e\x5f\x62\x6f\x6c\x64", label: "Delay Dokumen Bold (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x64\x6f\x6b\x75\x6d\x65\x6e\x5f\x61\x6c\x6c", label: "Delay Dokumen Saya (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x70\x70\x6e", label: "Delay e-Faktur Non Retur (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x70\x70\x6e\x5f\x72\x65\x74\x75\x72", label: "Delay e-Faktur Retur (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x62\x70\x70\x75", label: "Delay BPPU & BPNR (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x70\x65\x6e\x67\x6b\x72\x65\x64\x69\x74\x61\x6e", label: "Delay Pengkreditan Faktur (ms)" },
              { key: "\x64\x65\x6c\x61\x79\x5f\x70\x65\x6d\x62\x61\x74\x61\x6c\x61\x6e", label: "Delay Pembatalan Faktur (ms)" }
            ];
            const delayInputs = {};
            modules.forEach(m => {
