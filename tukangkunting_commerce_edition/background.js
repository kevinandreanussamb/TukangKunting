(function() {
    ...
    async function checkStoredLicense(machineCode) {
        const {license_token: license_token} = await chrome.storage.local.get("license_token");
        if (!license_token) return {
            ok: false
        };
        return await verifyLicenseToken(license_token, machineCode);
    }
    async function generateMachineCode() {
        let {install_id: install_id} = await chrome.storage.local.get("install_id");
        if (!install_id) {
    ...
    async function verifyLicenseToken(token, machineCode) {
        try {
            ...
            if (payload.machineCode !== machineCode) return {
                ok: false,
                reason: "\x6d\x61\x63\x68\x69\x6e\x65\x20\x63\x6f\x64\x65\x20\x74\x69\x64\x61\x6b\x20\x63\x6f\x63\x6f\x6b"
            };
            if (Date.now() > payload.expiry) return {
                ok: false,
                reason: "\x6c\x69\x73\x65\x6e\x73\x69\x20\x73\x75\x64\x61\x68\x20\x65\x78\x70\x69\x72\x65\x64"
            };
            return {
                ok: true,
    ...
    function formatExpiry(ts) {
        return new Date(ts).toLocaleDateString("\x69\x64\x2d\x49\x44", {
    ...
    function daysLeft(ts) {
        return Math.max(0, Math.ceil((ts - Date.now()) / 864e5));
    }
    async function ensureActivation(tab) {
        ...
        return new Promise(resolve => {
            chrome.scripting.executeScript({
                ...
                func: machineCode => {
                    ...
                    return new Promise(resolvePrompt => {
                        ...
                        submitBtn.onclick = () => {
                            ...
                            if (!token) {
                                showError("Masukkan license token terlebih dahulu.");
                                return;
                            }
                            submitBtn.disabled = true;
                            submitBtn.innerHTML = '<span class="tka-spinner"></span>Memverifikasi...';
                        ...
                        window.__tukangActivationShowSuccess = (expiry, daysRemaining) => {
                            ...
                            const expDate = new Date(expiry).toLocaleDateString("\x69\x64\x2d\x49\x44", {
                            ...
                            sTitle.className = "tka-success-title";
                            sTitle.textContent = "🎉 Lisensi Aktif";
                            successPanel.appendChild(sTitle);
                            [ {
                                ...
                                lbl.className = "tka-success-label";
                                lbl.textContent = r.label;
                                const val = document.createElement("span");
                                val.className = "tka-success-value" + (r.cls ? " " + r.cls : "");
                                val.textContent = r.value;
                                row.append(lbl, val);
                                successPanel.appendChild(row);
                            });
                            const barTrack = document.createElement("div");
                            barTrack.className = "tka-success-bar-track";
                            const barFill = document.createElement("div");
                            barFill.className = "tka-success-bar-fill";
                            barFill.style.width = "0%";
                            barFill.style.background = barColor;
                            barTrack.appendChild(barFill);
                            successPanel.appendChild(barTrack);
                            requestAnimationFrame(() => {
                                setTimeout(() => {
                                    barFill.style.width = Math.min(100, Math.max(5, daysRemaining / 365 * 100)) + "%";
                                }, 100);
                            });
                            const sActions = document.createElement("div");
                            sActions.className = "tka-success-actions";
                            ...
                            sActions.appendChild(continueBtn);
                            successPanel.appendChild(sActions);
                        };
                        function rm() {
                            overlay.remove();
                            modal.remove();
                ...
                const {machineCode: mc, token: token} = results[0].result;
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
        if (msg.action === "\x63\x68\x65\x63\x6b\x4c\x69\x63\x65\x6e\x73\x65") {
            (async () => {
                try {
                    const machineCode = await generateMachineCode();
                    const {license_token: license_token} = await chrome.storage.local.get("license_token");
            ...
            chrome.scripting.executeScript({
                ...
                func: () => {
                    ...
                    return new Promise(resolvePrompt => {
                        ...
                        const BULAN_MAP = {};
                        BULAN_NAMES.forEach((n, i) => {
                            BULAN_MAP[n.toLowerCase()] = i + 1;
                        });
                        ...
                        function parseCSV(text) {
                            ...
                            if (missing.length > 0) return {
                                rows: [],
                                error: `Kolom berikut tidak ditemukan: ${missing.join(", ")}. Pastikan header CSV sesuai template.`
                            };
                            const rows = [];
                        ...
                        const overlay = document.createElement("div");
                        overlay.id = "tukang-faktur-overlay";
                        const modal = document.createElement("div");
                        modal.id = "tukang-faktur-modal";
                        const header = document.createElement("div");
                        header.className = "tkcsv-header";
                        const title = document.createElement("div");
                        title.className = "tkcsv-title";
                        title.textContent = "🧾 Pengkreditan Faktur";
                        const closeBtn = document.createElement("button");
                        closeBtn.className = "tkcsv-close";
                        closeBtn.textContent = "✕";
                        closeBtn.onclick = () => {
                            rm();
                            resolvePrompt(null);
                        ...
                        header.append(title, closeBtn);
                        const body = document.createElement("div");
                        body.className = "tkcsv-body";
                        const desc = document.createElement("div");
                        desc.className = "tkcsv-desc";
                        ...
                        btnRow.className = "tkcsv-btn-row";
                        const dlBtn = document.createElement("button");
                        dlBtn.className = "tkcsv-btn tkcsv-btn-outline";
                        dlBtn.innerHTML = "📥 Download Template";
                        dlBtn.onclick = () => {
                            downloadCSV(generateTemplateCSV(), "template_pengkreditan_faktur.csv");
                        };
                        const uploadBtn = document.createElement("button");
                        uploadBtn.className = "tkcsv-btn tkcsv-btn-outline";
                        ...
                        uploadBtn.onclick = () => {
                            const fi = document.createElement("input");
                                ...
                                reader.onload = ev => {
                                    uploadBtn.innerHTML = "📎 Upload CSV";
                                    processCSVText(ev.target.result, file.name);
                                };
                                reader.onerror = () => {
                                    uploadBtn.innerHTML = "📎 Upload CSV";
                                    showStatus("Gagal membaca file.", "err");
                                };
                        ...
                        statusBox.className = "tkcsv-status";
                        const tableWrap = document.createElement("div");
                        tableWrap.className = "tkcsv-table-wrap";
                        body.append(desc, btnRow, statusBox, tableWrap);
                        const footer = document.createElement("div");
                        footer.className = "tkcsv-footer";
                        const summary = document.createElement("div");
                        summary.className = "tkcsv-summary";
                        summary.style.display = "none";
                        const cancelBtn = document.createElement("button");
                        cancelBtn.className = "tkcsv-btn tkcsv-btn-outline";
                        ...
                        processBtn.className = "tkcsv-btn tkcsv-btn-success";
                        processBtn.textContent = "🚀 Proses Semua";
                        ...
                        function processCSVText(text, filename) {
                            const {rows: rows, error: error} = parseCSV(text);
                            if (error) {
                            ...
                            if (rows.length === 0) {
                                showStatus("❌ File CSV tidak berisi data.", "err");
                                tableWrap.style.display = "none";
                                summary.style.display = "none";
                                processBtn.disabled = true;
                                return;
                            ...
                            table.className = "tkcsv-table";
                            const thead = document.createElement("thead");
                            const headTr = document.createElement("tr");
                            [ "#", ...HEADER_LABELS, "Status" ].forEach(h => {
                            ...
                            parsedRows.forEach((r, idx) => {
                                ...
                                HEADERS.forEach(h => {
                                    ...
                                    td.className = cellErrors.length > 0 ? "cell-err" : "cell-ok";
                                    tr.appendChild(td);
                                });
                                const tdStatus = document.createElement("td");
                                if (r._errors.length === 0) {
                                    tdStatus.textContent = "✓ Valid";
                                    tdStatus.className = "cell-ok";
                                } else {
                                    tdStatus.textContent = "✗ " + r._errors[0];
                                    tdStatus.className = "cell-err";
                                    tdStatus.title = r._errors.join("\n");
                                }
                            ...
                            if (errorCount === 0) {
                                showStatus(`✅ <strong>${filename}</strong> — ${validCount} baris terdeteksi, semua valid. Siap diproses!`, "ok");
                                processBtn.disabled = false;
                            } else if (validCount > 0) {
                                showStatus(`⚠️ <strong>${filename}</strong> — ${validCount} baris valid, <strong>${errorCount} baris bermasalah</strong> (akan dilewati).`, "warn");
                                processBtn.disabled = false;
                            } else {
                                showStatus(`❌ <strong>${filename}</strong> — Semua ${errorCount} baris bermasalah. Perbaiki CSV dan upload ulang.`, "err");
                                processBtn.disabled = true;
                            }
                        }
                        function rm() {
            ...
            chrome.scripting.executeScript({
                ...
                func: () => {
                    ...
                    return new Promise(resolvePrompt => {
                        ...
                        const overlay = document.createElement("div");
                        overlay.id = "tukang-batal-overlay";
                        const modal = document.createElement("div");
                        modal.id = "tukang-batal-modal";
                        const header = document.createElement("div");
                        header.className = "tkcsv-header";
                        const title = document.createElement("div");
                        title.className = "tkcsv-title";
                        title.textContent = "🚫 Pembatalan Faktur";
                        const closeBtn = document.createElement("button");
                        closeBtn.className = "tkcsv-close";
                        closeBtn.textContent = "✕";
                        closeBtn.onclick = () => {
                        ...
                        passWrap.className = "tkcsv-password-wrap";
                        const passLabel = document.createElement("label");
                        passLabel.className = "tkcsv-password-label";
                        passLabel.textContent = "Passphrase";
                        const passInput = document.createElement("input");
                        passInput.type = "password";
                        passInput.className = "tkcsv-password-input";
                        passInput.placeholder = "Masukkan passphrase sertifikat...";
                        const passHint = document.createElement("div");
                        passHint.className = "tkcsv-password-hint";
                        passHint.textContent = "Passphrase digunakan untuk proses penandatanganan pembatalan faktur. (Passphrase tidak disimpan atau dikirim ke mana pun, hanya digunakan secara lokal di browser Anda saat memproses faktur yang valid)";
                        ...
                        dlBtn.innerHTML = "📥 Download Template";
                        dlBtn.onclick = () => {
                            downloadCSV(generateTemplateCSV(), "template_pembatalan_faktur.csv");
                        };
                        const uploadBtn = document.createElement("button");
                        uploadBtn.className = "tkcsv-btn tkcsv-btn-outline";
                        uploadBtn.innerHTML = "📎 Upload CSV";
                        ...
                        const processBtn = document.createElement("button");
                        processBtn.className = "tkcsv-btn tkcsv-btn-danger";
                        ...
                        function processCSVText(text, filename) {
                            ...
                            summary.innerHTML = `<span class="tkcsv-dot tkcsv-dot-green"></span> ${validCount} valid` + (errorCount > 0 ? ` &nbsp; <span class="tkcsv-dot tkcsv-dot-red"></span> ${errorCount} error` : "");
                            if (errorCount === 0) {
                                showStatus(`✅ <strong>${filename}</strong> — ${validCount} faktur terdeteksi, semua valid. Siap dibatalkan!`, "ok");
                                processBtn.disabled = false;
                            } else if (validCount > 0) {
                                showStatus(`⚠️ <strong>${filename}</strong> — ${validCount} faktur valid, <strong>${errorCount} bermasalah</strong> (akan dilewati).`, "warn");
                                processBtn.disabled = false;
                            } else {
                                showStatus(`❌ <strong>${filename}</strong> — Semua ${errorCount} baris bermasalah. Perbaiki CSV dan upload ulang.`, "err");
                                processBtn.disabled = true;
        ...
        chrome.scripting.executeScript({
            ...
            func: (expiryStr, daysRemaining) => {
                ...
                return new Promise(resolve => {
                    ...
                    container.id = "doc-select-modal";
                    const header = document.createElement("div");
                    header.className = "tukang-header";
                    const brand = document.createElement("div");
                    brand.className = "tukang-brand";
                    const iconBox = document.createElement("div");
                    iconBox.className = "tukang-icon";
                    iconBox.textContent = "📄";
                    const brandText = document.createElement("div");
                    const titleEl = document.createElement("div");
                    titleEl.className = "tukang-title";
                    titleEl.textContent = "Tukang Dokumen";
                    const subtitleEl = document.createElement("div");
                    subtitleEl.className = "tukang-subtitle";
                    subtitleEl.textContent = "Document Automation Tool";
                    brandText.append(titleEl, subtitleEl);
                    brand.append(iconBox, brandText);
                    const headerActions = document.createElement("div");
                    headerActions.className = "tukang-header-actions";
                    const settingsBtn = document.createElement("button");
                    settingsBtn.className = "tukang-icon-btn";
                    ...
                    const closeBtn = document.createElement("button");
                    closeBtn.className = "tukang-icon-btn close-btn";
                    ...
                    const body = document.createElement("div");
                    body.className = "tukang-body";
                    const selectLabel = document.createElement("div");
                    selectLabel.className = "tukang-label";
                    selectLabel.textContent = "Jenis Dokumen";
                    ...
                    select.className = "tukang-select";
                    opts.forEach(o => {
                    ...
                    divider.className = "tukang-divider";
                    const submit = document.createElement("button");
                    submit.className = "tukang-submit";
                    ...
                    const footer = document.createElement("div");
                    footer.className = "tukang-footer";
                    const licenseInfo = document.createElement("div");
                    licenseInfo.className = "tukang-license-info";
                    const licenseLeft = document.createElement("div");
                    licenseLeft.className = "tukang-license-left";
                    let dotColor = "green";
                    if (daysRemaining <= 3) dotColor = "red"; else if (daysRemaining <= 7) dotColor = "orange";
                    const dot = document.createElement("div");
                    dot.className = "tukang-license-dot " + dotColor;
                    const licText = document.createElement("div");
                    licText.className = "tukang-license-text";
                    licText.textContent = "s/d " + expiryStr;
                    licenseLeft.append(dot, licText);
                    const badge = document.createElement("span");
                    badge.className = "tukang-license-badge " + dotColor;
                    badge.textContent = daysRemaining + " hari";
                    licenseInfo.append(licenseLeft, badge);
                    ...
                    settingsBtn.onclick = () => {
                        ...
                        settingsOverlay.id = "tukang-settings-overlay";
                        const settingsBox = document.createElement("div");
                        settingsBox.id = "settings-modal";
                        const sHeader = document.createElement("div");
                        sHeader.className = "settings-header";
                        const sTitle = document.createElement("div");
                        sTitle.className = "settings-title";
                        sTitle.innerHTML = `<span style="font-size:14px;opacity:.7">⚙️</span> Pengaturan`;
                        const sCloseBtn = document.createElement("button");
                        sCloseBtn.className = "tukang-icon-btn close-btn";
                        sCloseBtn.title = "Close";
                        ...
                        sBody.className = "settings-body";
                        const delaySection = document.createElement("div");
                        delaySection.className = "settings-section-title";
                        delaySection.innerHTML = `⏱️ Pengaturan Delay`;
                        ...
                        const modules = [ {
                            ...
                            label: "Delay e-Faktur Non Retur (ms)"
                        }, {
                            key: "\x64\x65\x6c\x61\x79\x5f\x70\x70\x6e\x5f\x72\x65\x74\x75\x72",
                            label: "Delay e-Faktur Retur (ms)"
                        }, {
                            key: "\x64\x65\x6c\x61\x79\x5f\x62\x70\x70\x75",
                            label: "Delay BPPU & BPNR (ms)"
                        }, {
                            key: "\x64\x65\x6c\x61\x79\x5f\x70\x65\x6e\x67\x6b\x72\x65\x64\x69\x74\x61\x6e",
                            label: "Delay Pengkreditan Faktur (ms)"
                        }, {
                            key: "\x64\x65\x6c\x61\x79\x5f\x70\x65\x6d\x62\x61\x74\x61\x6c\x61\x6e",
                        ...
                        modules.forEach(m => {
                            ...
                            sBody.appendChild(field);
                            delayInputs[m.key] = input;
                        });
                        chrome.storage.local.get([ ...modules.map(m => m.key), "passphrase" ], res => {
                            modules.forEach(m => {
                                delayInputs[m.key].value = res[m.key] ?? 0;
                            });
                            passInput.value = res.passphrase ?? "";
                        });
                        const sFooter = document.createElement("div");
                        sFooter.className = "settings-footer";
                        const sCancelBtn = document.createElement("button");
                        sCancelBtn.className = "btn-secondary";
                        sCancelBtn.textContent = "Batal";
                        sCancelBtn.onclick = () => {
                            settingsBox.remove();
                            settingsOverlay.remove();
                        };
                        const sSaveBtn = document.createElement("button");
                        sSaveBtn.className = "btn-primary";
                        sSaveBtn.textContent = "Simpan";
                        sSaveBtn.onclick = () => {
                            const saveObj = {};
                            modules.forEach(m => {
                                saveObj[m.key] = parseInt(delayInputs[m.key].value) || 0;
                            });
            ...
            if (sel === "Pengkreditan Faktur") {
                ...
                chrome.scripting.executeScript({
                    ...
                    args: [ csvData ],
                    func: list => {
                        window.__pengkreditanFakturList = list;
                    }
                }, () => {
                    chrome.scripting.executeScript({
                        target: {
            ...
            let file = null;
            if (sel === "Dokumen Saya (Bold Only)") file = "m0d_a3f8c1.js";
            if (sel === "Dokumen Saya") file = "m0d_b7d2e4.js";
            if (sel === "e-Faktur (Non Retur)") file = "ppn.js";
            if (sel === "e-Faktur (Retur)") file = "ppn_retur.js";
            if (sel === "BPPU & BPNR") file = "m0d_e2c7a0.js";
            if (file) {
                chrome.scripting.executeScript({
                    target: {
                        tabId: tab.id
                    },
                    files: [ "\x6c\x69\x62\x73\x2f\x6a\x71\x75\x65\x72\x79\x2d\x33\x2e\x37\x2e\x30\x2e\x6d\x69\x6e\x2e\x6a\x73", file ]
                });
            }
        });
    });
