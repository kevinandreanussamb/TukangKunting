(async function () {
  ...
  console.log("[Pengkreditan Faktur] Module injected");
  ...
  try {
    ...
    console.log("[Pengkreditan Faktur] list:", list);

    if (!list.length) {
      console.warn("[Pengkreditan Faktur] Tidak ada data untuk diproses.");
      alert("Tidak ada data pengkreditan untuk diproses.");
      return;
    }

    const rawDelay = await TK.Storage.getDelay("delay_pengkreditan", 1500);
    const delay = Math.max(Number(rawDelay) || 1500, 500);

    console.log("[Pengkreditan Faktur] delay:", delay);
    ...
    if (!TK.UI) throw new Error("TK.UI tidak tersedia");
    if (!TK.Batch) throw new Error("TK.Batch tidak tersedia");
    if (!TK.DOM) throw new Error("TK.DOM tidak tersedia");
    if (!TK.CSV) throw new Error("TK.CSV tidak tersedia");
    ...
    console.log("[Pengkreditan Faktur] UI created");

    const state = TK.Batch.createBatchState();

    console.log("[Pengkreditan Faktur] Batch state created", state);

    ui.onCancel(() => {
      state.cancelled = true;
      ui.log("User membatalkan proses.");
      console.log("[Pengkreditan Faktur] cancelled by user");
    });

    ui.onPauseToggle((paused) => {
      state.paused = paused;
      ui.log(paused ? "Paused by user." : "Resumed by user.");
      console.log("[Pengkreditan Faktur] paused:", paused);
    });
    ...
    function waitForFilteredRows(searchText, timeoutMs = 12000) {
      return new Promise((resolve) => {
        const start = Date.now();
        let lastRows = [];
        ...
        const timer = setInterval(() => {
          ...
          if (tbody) {
            lastRows = Array.from(tbody.querySelectorAll("tr"));

            const matched = lastRows.some((row) => {
              return row.textContent.includes(searchText);
            });
            ...
            const hasEmptyText =
              ...
              bodyText.includes("data tidak ditemukan") ||
              bodyText.includes("no records") ||
              bodyText.includes("no data");

            if (
              emptyMessage ||
              (lastRows.length === 1 && hasEmptyText)
            ) {
              clearInterval(timer);
              resolve({
                ok: false,
          ...
          if (Date.now() - start > timeoutMs) {
            ...
            resolve({
              ok: false,
              rows: lastRows,
              reason: lastRows.length ? "not_matched_timeout" : "empty_timeout",
            });
          }
        }, 200);
      });
    ...
    ui.log("Debug: module siap, akan masuk loop.");
    console.log("[Pengkreditan Faktur] before loop");

    for (let i = 0; i < list.length; i++) {
      console.log("[Pengkreditan Faktur] loop index", i);
      ...
      ui.log(`Mulai proses ${i + 1}/${list.length}: ${noFaktur || "(kosong)"}`);
      ...
      try {
        ...
        const filterResult = await waitForFilteredRows(noFaktur, 12000);

        if (!filterResult.ok) {
          throw new Error(
            `Data tidak ditemukan setelah filter nomor faktur ${noFaktur}. ` +
              `Periksa nomor faktur, masa pajak faktur, tahun pajak faktur, atau status faktur. ` +
        ...
        if (!targetRow) {
          throw new Error(
            `Nomor faktur ${noFaktur} tidak ditemukan di tabel setelah filter.`
          );
        }
    ...
    console.error("[Pengkreditan Faktur] Fatal error:", fatalErr);

    try {
      const panel = document.getElementById("tk-panel");
      const log = document.getElementById("tkLog");
      const status = document.getElementById("tkStatusText");

      if (status) status.textContent = "Fatal Error";
      if (log) {
        log.innerHTML =
          `[${new Date().toLocaleTimeString("id-ID")}] Fatal: ${
            fatalErr.message || fatalErr
          }<br>` + log.innerHTML;
      }

      if (!panel) {
        alert(`Pengkreditan Faktur error: ${fatalErr.message || fatalErr}`);
      }
    } catch {
      alert(`Pengkreditan Faktur error: ${fatalErr.message || fatalErr}`);
    }
